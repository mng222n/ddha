...
{   
   ...
   {  
      ...
      {  TransactionServer t=new TransactionServer(sip_provider,msg,null);
         ...
         System.out.println("[MDEBUG] processRequestToLocalServer:");
   ...
   {  printLog("inside getTargets(msg)",LogLevel.LOW);
      ...
      {  printLog("user "+user+" not found",LogLevel.HIGH);
      	//@skminh added for DHT overlay
      	lines = location_service.DHThasUser(username); 
      	if (lines == null) {
      		printLog("DHT: user "+username+" not found",LogLevel.HIGH);
      		return targets;
      	 } else {
           	isDHTUsed = true;
           	printLog("DHT: user "+username+" found",LogLevel.HIGH);
      	 }
      }
      
      SipURL to_url=msg.getToHeader().getNameAddress().getAddress();
      //do this if locally
    //@skminh added for DHT overlay - 23.07.08
      if ( !isDHTUsed ) {
      ...
      //only do this if have use DHT overlay in order to lookup
      //@skminh added for DHT overlay - 23.07.08
		if (lines != null ) {
			printLog("DHT: line !=null",LogLevel.HIGH);  
			ListIterator<String> li = lines.listIterator();
			String line = null;
			
			while (li.hasNext()) {
				line = (String)li.next();
				strTemp= "sip:" + username + "@" + line;
				targets.addElement(strTemp);
				printLog( "MDEBUG-DHT: " + strTemp, LogLevel.HIGH );
			}
			return targets;
		}
		printLog("DHT: line=null!!!",LogLevel.HIGH); 
   ...
   {  ToHeader th=msg.getToHeader();
   	  ContactHeader cth=msg.getContactHeader();
      if (th==null)  
      ...
      //@skminh added for DHT
      SipURL contact_uri=cth.getNameAddress().getAddress();
      String contact_user=contact_uri.getUserName();
      String contact_host=contact_uri.getHost();
      int contact_port=contact_uri.getPort();

      String dhtuser;
      	
      if (contact_port < 0) //so port is default 5060
    	  dhtuser =contact_user+"@"+contact_host+":5060";
      else
    	  dhtuser =contact_user+"@"+contact_host+":"+Integer.toString(contact_port);
      System.out.println("[MDEBUG]dhtuser:" + dhtuser );
      int ret = location_service.DHTaddUser(dhtuser);
      if (ret < 0 )
      	printLog("DHT Overlay: add "+user+" has error.",LogLevel.HIGH);
      else
      	printLog("DHT Overlay: new user "+dhtuser+" added.",LogLevel.HIGH);
      //---
