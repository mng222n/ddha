 ...
 * 
 * Mofified by skminh - 22.07.08
 * Using distributed database upon DHT
 */
...
import com.int2.overlay.IOverlayImpl;
...
public class LocationServiceImpl implements LocationService
{
   ...
   /** Declare an instance in order to access DHT overlay via XML-RPC */
   IOverlay iover;
   boolean isIOverStarted;
   ...
   {  filename=file_name;
      users=new Hashtable();
      load();
      isIOverStarted = false;
      startIOverlay();
   }
   
   public void startIOverlay() {
	   iover = new IOverlayImpl();
	   try {
		   iover.setGateway(Constants.getDHTGatewayIP(), Constants.DHTGatewayPort);
	   } catch (Exception e) {
   ...
   //@skminh added
   /** Whether a user is present in the DHT and can be used as key.
    * @param user the user name
    * @return true if the user name is present as key */
  public List<String> DHThasUser(String username)
  {  
		if (isIOverStarted == false) return null;
		//get
		List<String> lines = null;
		try {
			lines = iover.get(username, new Integer(10) );
			if (lines == null)
				System.out.println("DHT: Get fail");
			else
				System.out.println("DHT: Get succeed");
		} catch (Exception e) {
  ...
  public boolean DHThasContact(String dhtuser)
  {  
	    if (isIOverStarted == false) {
			System.out.println("[MDEBUG]DHThasContact: isIOverStarted = false");
			return false;
		}
		//get
		List<String> lines = null;
		String[] strArr = Constants.getTarget(dhtuser);
		try {
			lines = iover.get(strArr[0], new Integer(10) );
			if (lines == null) {
				//System.out.println("[MDEBUG]DHThasContact: lines = null");
				return false;
				
			} else {
				//System.out.println("[MDEBUG]DHThasContact: get succed");
				...
				while (li.hasNext()) {
					line = (String)li.next();
					System.out.println("[MDEBUG]strArr[0]:"+strArr[0]+",strArr[1]:"+strArr[1]+",line:"+line);
					if ( line.equalsIgnoreCase(strArr[1])) {
						System.out.println("[MDEBUG]DHThasContact: contact exist!");
						return true;
					}
				}
			}	
   ...
   //@skminh added
   /**
    * Put user/ip into DHT overlay
    * @param user
    * @return
    */
   public int DHTaddUser(String dhtuser)
   {  
	int ret = -1;
	if (isIOverStarted == false) return -1;
	System.out.println("[MDEBUG]DHTaddUser:"+dhtuser);
	String[] strArr = Constants.getTarget(dhtuser);
    if (DHThasContact(dhtuser)==true) return -1;
	try {
		ret = iover.put(strArr[0], strArr[1], new Integer(3600) ); //ttl=3600sec
		if (ret < 0 )
			System.out.println("Put fail (Put user)");
		else
			System.out.println("Put succeed (Put user)");
		
		ret = iover.put(Constants.MASS08_SIP_KEY, dhtuser, new Integer(3600) ); //ttl=3600sec
		if (ret < 0 )
			System.out.println("Put fail (Put4monitoring)");
		else
			System.out.println("Put succeed (Put4monitoring)");
