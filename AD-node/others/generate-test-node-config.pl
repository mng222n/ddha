...
sub start_node {
    ...
    my %variable_map = ("NodeID"      => "157.159.103.55:$port",
                        "GatewayID"   => $gateway,
                        "CacheDir"    => $blocksdir,
                        "GatewayPort" => ($port+2));
...
my $nodes_addr = "157.159.103.84";
my $gateway_addr = "157.159.103.74";
my $next_port = 3630;
my $node_count = 0;
my $gateway;
my @pids;
...
sub start_node_mi {
	...
	$gateway = "$gateway_addr:$next_port"; 
	#push @pids, &start_node ($gateway, "localhost", $next_port);
	#push @pids, &start_node ($gateway, $gateway_addr, $next_port);
	print "Gateway is ready started\n";
    }
    else {
	push @pids, &start_node ($gateway, $nodes_addr, $next_port);
    }
    $next_port += 3;
    ++$node_count;
}
